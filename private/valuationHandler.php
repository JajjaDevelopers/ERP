...
$quantityInSql = $conn->prepare("INSERT INTO nucafe_inventory (inventory_reference, grade_id, qty_in, price_ugx) VALUES (?, ?, ?, ?)"); // nucafe inventory
...
for ($x=0; $x < count($allGradeQty); $x++ ) {
    ...
    if ($gradeQty > 0){
        $quantityInSql->bind_param("ssdd", $valuationNo, $gradeName, $gradeQty, $gradePrice);
